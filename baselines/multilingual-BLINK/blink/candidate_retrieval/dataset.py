...
def eval(testset, system_pred, nel=False):
    ...
    for doc_name, content in testset.items():
        ...
        pred += [c["pred"][0] for c in system_pred[doc_name]]  # the predicted named entity
    ...
    if nel:
        ...
        for doc_name, content in testset.items():
            total_discovered_mentions += np.sum(len(ment) for ment in content[0]["ments_per_sent_flair"])
...
class FetchCandidateEntities(object):
    ...
    def __init__(self, p_e_m_data_path="data/basic_data/p_e_m_data/"):
        ...
        self.lower_org = []
        self.lower_lower = []
        self.p_e_m = pickle.load(open(os.path.join(p_e_m_data_path, "p_e_m_dict.pickle"), "rb"))
        self.p_e_m_lower = pickle.load(open(os.path.join(p_e_m_data_path, "p_e_m_lower_dict.pickle"), "rb"))
