...
def get_wikidata_id_from_link_name(link):
    ...
    if linktitle2id is None:
        ...
        if os.path.isfile(path_to_file):
            ...
            subprocess.call("./blink/candidate_retrieval/scripts/generate_wiki2wikidata_mapping.sh")
            linktitle2id = pickle.load(open(path_to_file, "rb"))
...
def get_datasets(get_test_dataset=False, get_pregenereted_candidates_wikidata_id=False):
    ...
    datadir = os.path.join(train_and_benchmarking_data_dir, "generated/test_train_data/")
    conll_path = os.path.join(train_and_benchmarking_data_dir, "basic_data/test_datasets/")
    person_path = os.path.join(train_and_benchmarking_data_dir, "basic_data/p_e_m_data/persons.txt")
...
def get_sent_context(mention, key, solr_escaped=True):
    ...
    if key.endswith("next"):
        if key.endswith("prev_next"):
            res = "{} {} {}".format(
                "" if mention[mention_data_key][0] is None else mention[mention_data_key][0],
                mention[mention_data_key][1],
                "" if mention[mention_data_key][2] is None else mention[mention_data_key][2],
            )
        else:
            res = "{} {}".format(
                mention[mention_data_key][1],
    ...
    for ds_name, dataset in dev_datasets:
        ...
        for doc_name, content in dataset.items():
            ...
            for m in content:
                ...
                m["query_context"] = "{} {} {}".format(left_context, m["mention"], right_context).strip()
...
def write_candidate_generation_results_for_a_run_to_file(run, results_dump_file_path):
    ...
    with open(txt_file_path, "a+") as file:
        ...
        res = " --- ".join(["{} - {:.2f}".format(key, run[1][key]) for key in sorted(run[1].keys())])
        file.write("{} === {}\n".format(res, id_))


def write_candidate_generation_execution_time_to_file(results_dump_file_path, execution_time):
    txt_file_path = "{}.txt".format(results_dump_file_path)
...
def write_candidate_generation_results_to_file(runs, results_dump_file_path, execution_time=None):
    runs.sort(key=lambda run: -run[1]["overall"])

    for run in runs:
        write_candidate_generation_results_for_a_run_to_file(run, results_dump_file_path)

    if execution_time is not None:
        write_candidate_generation_execution_time_to_file(results_dump_file_path, execution_time)
