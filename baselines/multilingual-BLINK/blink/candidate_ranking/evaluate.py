...
def evaluate_model_on_dataset(
    ...
    for input_ids, input_mask, segment_ids, label_ids, entity_mask in tqdm(dataloader, desc="Evaluating"):
...
def evaluate(parameters, logger=None):
    ...
    candidates_key = (
        "pregenerated_candidates" if parameters["evaluate_with_pregenerated_candidates"] else "candidates"
    )
    gold_key = "pregenerated_gold_pos" if parameters["evaluate_with_pregenerated_candidates"] else "gold_pos"
    ...
    for eval_dataset_name in eval_datasets:
        ...
        eval_samples_filtered = utils.filter_samples(eval_samples, parameters["top_k"], gold_key)
        logger.info("Retained {} out of {} samples".format(len(eval_samples_filtered), len(eval_samples)))
        ...
        if parameters["output_eval_file"] is None:
            output_eval_file = os.path.join(parameters["path_to_model"], "eval_results.txt")
        else:
        ...
        if logger != None:
            logger.info(
                "The execution for dataset {} took {} minutes".format(eval_dataset_name, execution_time)
            )
        else:
            print("The execution for dataset {} took {} minutes".format(eval_dataset_name, execution_time))

    if logger != None:
        logger.info("The execution for dataset {} took {} minutes".format(eval_dataset_name, execution_time))
    else:
