...
class BertForReranking(PreTrainedModel):
    r"""
    Inputs:
...
class BertReranker:
    ...
    def rerank(self, mentions, sentences):
        ...
        dataloader = DataLoader(tensor_data, sampler=sampler, batch_size=p["evaluation_batch_size"])
        ...
        for input_ids, input_mask, segment_ids, mention_ids, entity_mask in tqdm(
            ...
            with torch.no_grad():
                logits = self.model(input_ids, segment_ids, input_mask, entity_mask=entity_mask)[0]
            ...
            for idx, mention_idx in enumerate(mention_ids):
                ...
                mentions[mention_idx]["prob_assigned_to_candidate"] = probs[idx][pred].item()
    ...
    def get_scheduler_and_optimizer(self, parameters, train_tensor_data, logger):
        ...
        num_warmup_steps = int(num_train_optimization_steps * parameters["warmup_proportion"])
        ...
        scheduler = get_linear_schedule_with_warmup(
            ...
            num_training_step=num_train_optimization_steps,
        )
    ...
    def _get_candidate_representation(
        ...
        tokens = ["[CLS]"] + context_tokens + ["[SEP]"] + candidate_desc_tokens + ["[SEP]"]
        segment_ids = [0] * (len(context_tokens) + 2) + [1] * (len(candidate_desc_tokens) + 1)
    ...
    def _select_field(samples, field):
        ...
        return [[cand[field] for cand in sample["candidate_features"]] for sample in samples]
    ...
    def _get_context_token_representation(
        ...
        mention_tokens = [start_token] + tokenizer.tokenize(sample[mention_text_key]) + [end_token]
    ...
    def _process_mentions_for_model(
        ...
        for idx, mention in enumerate(iter_):
            ...
            if sentences is not None:
                mention[context_key] = BertReranker._get_mention_context_end2end(mention, sentences)
            ...
            entity_mask = [1] * len(candidate_features) + [0] * (top_k - len(candidate_features))
        ...
        all_entity_masks = torch.tensor([s["entity_mask"] for s in processed_mentions], dtype=torch.float)
        ...
        if sentences is not None:
            ...
            all_label = torch.tensor([s["label"] for s in processed_mentions], dtype=torch.long)
        ...
        if logger != None:
            ...
            if sentences is not None:
                logger.info("all_mention_indices shape: {}".format(all_mention_indices.shape))
            else:
