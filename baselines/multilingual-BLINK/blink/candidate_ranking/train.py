...
def main(parameters):
    ...
    if os.path.exists(model_output_path) and os.listdir(model_output_path):
        print("Output directory ({}) already exists and is not empty.".format(model_output_path))
        ...
        if answer.strip() == "Y":
            ...
            raise ValueError(
                "Output directory ({}) already exists and is not empty.".format(model_output_path)
            )

    if not os.path.exists(model_output_path):
        os.makedirs(model_output_path)

    utils.write_to_file(os.path.join(model_output_path, "training_parameters.txt"), str(parameters))
    ...
    train_samples = utils.read_dataset(train_dataset_name, parameters["path_to_preprocessed_json_data"])
    train_samples_filtered = utils.filter_samples(train_samples, parameters["top_k"])
    logger.info("Retained {} out of {} samples".format(len(train_samples_filtered), len(train_samples)))
    ...
    dev_samples = utils.read_dataset(dev_dataset_name, parameters["path_to_preprocessed_json_data"])
    dev_samples_filtered = utils.filter_samples(dev_samples, parameters["top_k"])
    logger.info("Retained {} out of {} samples".format(len(dev_samples_filtered), len(dev_samples)))
    ...
    dev_dataloader = DataLoader(dev_tensor_data, sampler=dev_sampler, batch_size=evaluation_batch_size)
    ###
    ...
    optimizer, scheduler = reranker.get_scheduler_and_optimizer(parameters, train_tensor_data, logger)
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Batch")):
            ...
            loss, _ = model(input_ids, segment_ids, input_mask, label_ids, entity_mask=entity_mask)
            ...
            if (step + 1) % (
                parameters["print_tr_loss_opt_steps_interval"] * parameters["gradient_accumulation_steps"]
            ) == 0:
                logger.info(
                    "Step {} - epoch {} average loss: {}\n".format(
                        ...
                        epoch_idx,
                        tr_loss
                        / (parameters["print_tr_loss_opt_steps_interval"] * gradient_accumulation_steps),
                    )
                )
                tr_loss = 0
            ...
            if (step + 1) % (
                parameters["dev_evaluation_interval"] * gradient_accumulation_steps * train_batch_size
            ) == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--context_key", default="tagged_query_context_sent_prev_curr_next", type=str)
    parser.add_argument(
        "--lowercase_flag",
        action="store_true",
